...
cd /home/ec2-user
unzip -o instantclient-basiclite-linux.x64-19.3.0.0.0dbru.zip
unzip -o instantclient-sqlplus-linux.x64-19.3.0.0.0dbru.zip
...
chown ec2-user:ec2-user /home/ec2-user/FNCM_oracle_commands.sh
chown ec2-user:ec2-user /home/ec2-user/FNCM_oracle_database_objects.sql
chmod 755 /home/ec2-user/FNCM_oracle_commands.sh

# Set user passwords in SQL script file
sed -i -e "s/rds_password/$DBPassword/g" /home/ec2-user/FNCM_oracle_database_objects.sql

# Build the sqlplus command
Connection="/home/ec2-user/instantclient_19_3/sqlplus '$DBUser/$DBPassword@(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=$Endpoint)(PORT=$Port))(CONNECT_DATA=(SID=$DBName)))' @FNCM_oracle_database_objects.sql"
